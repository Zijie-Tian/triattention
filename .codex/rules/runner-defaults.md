# Critical Runner Defaults Execution Rule

## Rule Severity

This is a **must-follow execution rule** for Codex in this repository.

When running experiments, preparing launch commands, adjusting GPU placement, changing shard counts, selecting the conda environment, or modifying any durable execution default, Codex must treat this rule as **binding**, not advisory.

If an execution path conflicts with this rule, Codex must change the execution path to comply with this rule rather than bypassing it.
...
For standard project execution, Codex must assume the following:

1. **All durable execution defaults must be defined through `triattention/configs/shared/runner_defaults.yaml`.**
2. **Generated configs are derived artifacts and must not be treated as the canonical configuration surface.**
3. **Helper scripts are orchestration wrappers, not the authoritative place for core runtime defaults.**
4. **One-off CLI overrides are temporary exceptions for debugging only, not the accepted default workflow.**

Any workflow that primarily depends on:

- editing generated YAML by hand
- repeatedly passing `dispatch.py` overrides instead of updating defaults
- relying on shell-local environment variables as the durable configuration surface
- hardcoding core runtime defaults into helper scripts
...
- adjust dataset length defaults
- alter default worker behavior

Codex must first ask:

> Is this supposed to be a durable project default or just a disposable debug override?

If it is meant to be durable or repeatable, the change must be implemented in `runner_defaults.yaml`.

If it is explicitly temporary, Codex may use a one-off override, but should still note that the canonical default remains in `runner_defaults.yaml`.
