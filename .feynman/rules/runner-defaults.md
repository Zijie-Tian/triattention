...
If the requested behavior can be expressed through the existing configuration surfaces, the agent must implement it by editing config, **not** by editing Python launch code or shell wrappers.
...
Only change code when the required behavior is genuinely impossible to express with the current config surface or when the config plumbing itself is broken. In that case, the agent must explicitly note why a config-only change was insufficient.
